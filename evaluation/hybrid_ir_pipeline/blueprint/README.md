...
blueprint는 성능 결과가 아니라 데이터 설계 근거입니다. 해커톤 제출에서 이 문서는 "평가 데이터를 무작정 만든 것이 아니라, 증상군과 문진 질문, 방언 layer, 상태 패턴을 먼저 고정했다"는 근거로 사용합니다.

다음처럼 표현하는 것이 안전합니다.

```text
train_100_v2는 렌더링 전에 blueprint로 분포를 고정했고,
방언 RAG 근거가 있는 행과 단순 강원 말투 행을 분리해 관리했다.
```

다음처럼 표현하면 안 됩니다.

- blueprint 통과를 모델 성능으로 해석
- 모든 강원체 행이 방언 pack에 의해 근거화됐다고 주장

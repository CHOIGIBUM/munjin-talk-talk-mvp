...
`test/add-coverage` 브랜치에서 이 폴더는 일반 단위 테스트가 아니라 IR 성능 점검용 평가 보조 코드입니다. 백엔드/프론트 테스트와는 목적이 다르며, 후보 검색 품질과 LLM linker 선택 품질을 분리해서 보기 위해 둡니다.
...
실행 단계에 따라 외부 호출 여부가 다릅니다.

| 단계 | AWS/LLM 호출 | 설명 |
| --- | --- | --- |
| pipeline span 생성 | Bedrock 사용 가능 | 운영 파이프라인 기반 표준화와 의미 span 추출 |
| IR 후보 검색 | Titan embedding cache 또는 provider 설정에 의존 | BM25, vector, label signal 조합 |
| Nova Pro linker | Bedrock 사용 | top-20 후보 중 최종 표준 증상 선택 |
| `--skip-llm-judge` | linker 호출 생략 | 후보 검색 품질만 빠르게 확인 |

따라서 이 폴더의 평가 스크립트는 일반 CI 단위 테스트처럼 무심코 돌리는 대상이 아닙니다. AWS 권한, 비공개 IR 런타임 데이터, 비용 영향을 확인한 뒤 실행합니다.
...
주의할 점은 후보 검색 지표와 최종 선택 지표가 서로 다른 질문에 답한다는 것입니다. 후보 안에 정답이 있어도 linker가 고르지 못하면 최종 선택은 실패할 수 있고, 후보 검색만 높다고 원페이퍼 품질이 보장되는 것도 아닙니다.

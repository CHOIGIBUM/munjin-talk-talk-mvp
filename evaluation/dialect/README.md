...
이 폴더는 문진톡톡의 `backend/serverless/src/dialect_normalization.py`에 구현된 `normalize_dialect_text()`를 AWS Bedrock 환경에서 실행해, 사투리/구어체 문진 텍스트가 표준어 보조 문장으로 얼마나 잘 바뀌는지 평가하기 위한 예시입니다.
...
## 의미 보존 LLM judge까지 실행

문장 exact match는 너무 엄격할 수 있습니다. 예를 들어 `매우 힘들어요`와 `너무 힘들어요`는 의미가 거의 같지만 exact match는 틀릴 수 있습니다. 이때 Bedrock judge를 추가로 돌립니다. 비용이 더 발생합니다.

```bash
export DIALECT_EVAL_JUDGE_MODEL_ID=apac.amazon.nova-pro-v1:0

python evaluation/dialect/run_dialect_norm_eval.py \
  --input evaluation/dialect/data/dialect_norm_eval_200.jsonl \
  --output-dir evaluation/dialect/outputs/run_001_judge \
  --semantic-judge
```
...
```text
evaluation/dialect/outputs/run_001/
...
| `semantic_same_meaning_rate` | 선택적 LLM judge 기준 의미 보존 비율 |
| `no_added_fact_rate` | 선택적 LLM judge 기준 새 의학적 사실을 추가하지 않은 비율 |

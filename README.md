...
이 저장소는 화면 목업만 포함한 저장소가 아닙니다. React/Vite 프론트엔드와 AWS 서버리스 백엔드, Bedrock 기반 LLM 파이프라인, Pydantic 검증, BM25 + Titan Embedding 기반 Hybrid IR, DynamoDB 최소 세션 저장, S3 가명처리 artifact 저장 구조를 포함합니다.
...
| DynamoDB/S3 하이브리드 저장 | 구현됨. DynamoDB는 상태/pointer, S3는 가명처리 산출물 |
...
7. S3 artifact에 답변/trace/원페이퍼 저장
8. DynamoDB 세션 상태와 S3 pointer 갱신
9. 처리 trace 반환
...
flowchart LR
  ...
  S3["S3 Artifact Bucket<br/>redacted JSON"]
...
환자 음성 원본은 저장하지 않습니다. 브라우저가 Amazon Transcribe Streaming으로 음성을 직접 보내고, 확인된 텍스트만 백엔드 파이프라인에 전달합니다.

상세 필드 기준은 [docs/SECURITY_DATA_INVENTORY.md](docs/SECURITY_DATA_INVENTORY.md)를 참고합니다.
...
`sam deploy --guided`에서 `ArtifactsBucketName`에는 가명처리 문진 산출물을 저장할 S3 bucket 이름을 입력해야 합니다.
...
- S3 artifact bucket Lifecycle, KMS, Macie 점검
...
현재 구현에서 환자 음성은 S3에 저장하지 않습니다. DynamoDB에는 대기열과 상태, 마스킹 환자명, S3 artifact key만 저장하고, 문진 텍스트와 원페이퍼/안내문 JSON은 가명처리 후 S3 artifact로 저장합니다. 실제 개인정보를 입력하려면 인증, 권한 분리, 보존 기간, Macie 탐지 정책을 별도로 검토해야 합니다.

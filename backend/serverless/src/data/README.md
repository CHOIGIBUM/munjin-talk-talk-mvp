...
이 폴더의 데이터는 서비스 실행뿐 아니라 평가와 테스트의 전제 조건이기도 합니다.

| 자료 | 평가/테스트에서의 역할 |
| --- | --- |
| `domain_packs/respiratory.json` | 공개 가능한 도메인 규칙, status enum, 안전 플래그, alias/few-shot 연결 기준 |
| `fewshots/respiratory/*.json` | LLM 구조화 단계의 대표 예시와 회귀 검증 기준 |
| `dialect_packs/dialect_kangwon.json` | 사투리 RAG 평가와 운영 표준어 보조 변환의 힌트 source |
| `question_sets/default.json` | 프론트엔드 문진 흐름과 백엔드 질문셋 테스트의 기준 |
| 비공개 IR 데이터 3종 | Hybrid IR 표준 증상 매칭과 성능 평가의 실제 런타임 source |

공개 저장소만으로는 코드 구조, 질문셋, 도메인팩, 방언 RAG 구조를 검토할 수 있습니다. 다만 서울아산병원 질병백과 기반 원천 데이터와 파생 embedding cache는 공개하지 않으므로, 운영 수준의 Hybrid IR 증상 매칭이나 대량 성능 평가를 재현하려면 비공개 런타임 데이터를 별도로 배치해야 합니다.

심사위원에게는 이 구분이 중요합니다. 공개 저장소에 모든 원천 의료 데이터를 넣지 않았다는 것은 기능 누락이 아니라, 저작권과 데이터 공개 범위 때문에 코드와 산출물 기준만 공개한 설계입니다.

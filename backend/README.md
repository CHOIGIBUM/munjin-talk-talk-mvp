...
> **배포 시 주의사항**
>
> 본 저장소에는 의학 백과 원천 데이터, 파생 증상 인덱스, 임베딩 캐시 및 실제 평가 데이터셋이 포함되어 있지 않습니다. 로컬 실행 및 AWS 배포 전, 내부 비공개 저장소에서 해당 런타임 데이터를 복사하여 `backend/serverless/src/data/` 경로에 수동으로 배치해야 Hybrid IR 검색 기능이 정상 작동합니다.
